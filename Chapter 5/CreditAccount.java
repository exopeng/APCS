...
public class CreditAccount {
	...
	public double calculateMinimumMonthlyPayment() {
		...
		if (2 * this.calculateInterestAmountForMonth() < 25) {
			return 25;
		}
		
		return 2 * this.calculateInterestAmountForMonth();
	}
	...
	public int howManyMonthsUsingConstantPayment(double payment) {
		...
		int months = 0;
		if (payment <= this.calculateMinimumMonthlyPayment()) {
			throw new IllegalArgumentException("Will never be paid off");
			
		} else {
			double hypoBalance = this.getBalance();
			while (hypoBalance > 0) {
				months++;
				hypoBalance += (annualInterestRate / 12) * hypoBalance;
				hypoBalance -= payment;
			}
		}
		return months;
	...
	public String toString() {
		...
		return accountHolder + ", " + String.format("$%.2f", this.getBalance()) + ", " + this.getInterestRate() * 100 + "%";
	}
